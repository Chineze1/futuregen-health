...
const cellTone = (g: string) =>
  g === "SS" ? "bg-danger text-danger-foreground" : g === "AS" ? "bg-moderate text-moderate-foreground" : "bg-safe text-safe-foreground";

export function PunnettSquare({ result }: { result: PredictionResult }) {
  return (
    <div className="rounded-3xl bg-card p-5 shadow-card">
      <p className="mb-4 text-sm font-medium text-muted-foreground">Genetic cross (Punnett square)</p>
